...
class Parser:
    def __init__(self):
        self.regex = re.compile('<div class="post article" id="\d+"><p>(.*?)<\/p><div class="date"><div class="left_part">.*?<p>Le ([^-]*?) - .*? - par (.*?) (\(<a.*?<\/a>\))?<\/p><\/div>', re.MULTILINE)

    def parse(self, page):
        return [self.__createPostFromMatch(m) for m in self.regex.findall(page)]
